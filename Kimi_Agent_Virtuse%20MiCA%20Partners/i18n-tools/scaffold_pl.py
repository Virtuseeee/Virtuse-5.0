#!/usr/bin/env python3
"""Scaffold pl/<page>.html from the current (redesigned) EN root page.

Structural transforms only -- the copy stays English and is translated
afterwards by apply_pl_translations.py. Written for the post-redesign
...
Usage: python3 i18n-tools/scaffold_pl.py [page.html ...]   (default: all)
"""
import os
import re
import sys
...
NAV_SQUEEZE_CSS = """
/* ===== PL: compact nav text =====
   Polish nav labels run longer than English ("Przechowywanie", "Dane o
   Bitcoinie"), so at the default padding they wrap onto two lines at
...
/* Below ~1200px the twelve Polish labels still push the CTA off-screen. */
...
def rebuild_switchers(s, page):
    """Rewrite every .lang-menu-panel's options and add PL (active)."""
    ...
    s, nb = re.subn(r'(<button[^>]*class="lang-menu-btn"[^>]*>\s*)<span class="lang-flag">🇬🇧</span>EN',
                    r'\1<span class="lang-flag">🇵🇱</span>PL', s)
    if nb != n:
...
def scaffold(page):
    ...
    s, n = re.subn(r'<html lang="en"', '<html lang="pl"', s, count=1)
    if not n:
        fail(page + ": no <html lang=\"en\">")

    # hreflang: add pl right after es (404 has none)
    es_line = '<link rel="alternate" hreflang="es" href="%s/es/%s">' % (DOMAIN, page)
    if 'hreflang="pl"' in s:
        pass  # the EN page is already wired (wire_pl_into_existing.py)
    elif es_line in s:
        s = s.replace(es_line, es_line + '\n<link rel="alternate" hreflang="pl" href="%s/pl/%s">' % (DOMAIN, page), 1)
    elif page != "404.html":
        fail(page + ": es hreflang line not found")

    s = s.replace('<meta property="og:url" content="%s/%s">' % (DOMAIN, page),
                  '<meta property="og:url" content="%s/pl/%s">' % (DOMAIN, page))
    s = s.replace('<meta property="og:locale" content="en_US">', '<meta property="og:locale" content="pl_PL">')
    ...
    s = re.sub(r'<a href="(?:index\.html|/)" class="nav-logo">', '<a href="/pl/" class="nav-logo">', s)
    s = rewrite_attrs(s)
    ...
    s = s.replace('data-consultation-lang="en"', 'data-consultation-lang="pl"')
    s = s.replace("lang: 'en'", "lang: 'pl'")
    s = s.replace("JSON.stringify({ email: email, hp: hp })",
                  "JSON.stringify({ email: email, hp: hp, lang: 'pl' })")
    if "virtuse-newsletter" in s and "lang: 'pl'" not in s:
        fail(page + ": newsletter fetch found but lang 'pl' not set")
