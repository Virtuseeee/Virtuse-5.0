#!/usr/bin/env python3
"""
Add fr/<page> to sitemap.xml: inserts hreflang="fr" into every existing
<url> block for this page (the EN root entry, and the sk/, uk/, cs/, ru/,
de/ entries if they exist), then appends a new <url> block for fr/<page>
with the full set of alternates. Adapted from sitemap_add_de.py for the
7th language -- see i18n-tools/README.md "Adding the next language".

Usage: python3 sitemap_add_fr.py <page.html>
"""
import sys, re, os
...
# of language-folder prefix (root, sk/, uk/, cs/, ru/, de/, ...).
...
if any('hreflang="fr"' in m.group(0) for m in matches):
    print("Already has fr entry for", page)
    sys.exit(0)
...
fr_hreflang_line = f'    <xhtml:link rel="alternate" hreflang="fr" href="https://staging.virtuse.com/fr/{page}"/>\n'

def insert_fr_line(block_text):
    marker = f'    <xhtml:link rel="alternate" hreflang="x-default" href="https://staging.virtuse.com/{page}"/>\n'
    if marker not in block_text:
        return block_text
    return block_text.replace(marker, fr_hreflang_line + marker, 1)

# Rebuild the file: walk matches in order, patch each block in place, and
# after the LAST matched block insert the brand-new fr/<page> block.
out = []
last_end = 0
for m in matches:
    out.append(s[last_end:m.start()])
    out.append(insert_fr_line(m.group(0)))
    last_end = m.end()
...
if has_de:
    alt_lines.append(f'    <xhtml:link rel="alternate" hreflang="de" href="https://staging.virtuse.com/de/{page}"/>\n')
alt_lines.append(fr_hreflang_line)
...
fr_block = (
    f'  <url>\n'
    f'    <loc>https://staging.virtuse.com/fr/{page}</loc>\n'
...
s = s[:insert_at] + fr_block + s[insert_at:]

with open(SITEMAP, "w", encoding="utf-8") as f:
    f.write(s)
print("Updated sitemap for fr/" + page, f"(sk: {has_sk}, uk: {has_uk}, cs: {has_cs}, ru: {has_ru}, de: {has_de})")
