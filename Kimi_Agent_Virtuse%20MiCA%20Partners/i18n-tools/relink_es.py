...
footer links to pages that didn't have an es/ counterpart *yet* were
correctly left pointing at the English original (`../page.html`). Once
another page gets its es/ counterpart, those links should point at the
sibling Spanish page instead (`page.html`). This rewrites all of them in
one pass -- run once after a batch of pages is complete (same fixup as
relink_sk.py/relink_uk.py/relink_cs.py/relink_ru.py/relink_de.py/
relink_fr.py, now for es/).
...
(deliberately handled as its own separate step -- see relink_es_blog.py,
run only after es/blog.html exists).
"""
import glob
import os
...
pages = sorted(
    ...
    for p in glob.glob(os.path.join(ES_DIR, "*.html"))
    if os.path.basename(p) != "blog.html"
)

total_changes = 0
for page in pages:
    path = os.path.join(ES_DIR, f"{page}.html")
