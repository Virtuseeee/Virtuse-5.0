#!/usr/bin/env python3
"""Spanish variant (sed-derived from the French one) of port_homepage_redesign.py — same mechanical CSS/nav/
footer/newsletter port, adapted for es/ pages: Spanish aria-labels, the
...
.newsletter-form button:hover { opacity: 0.85; transform: translateY(-1px); }"""
    ...
    old_ham_html = '<button class="nav-hamburger" id="navHamburger" aria-label="Alternar menú" aria-expanded="false"><span></span><span></span><span></span></button>'
    new_ham_html = """<button class="nav-hamburger" id="navHamburger" aria-label="Alternar menú" aria-expanded="false">
    ...
    active_flag_label = (active_m.group(1) + active_m.group(2)) if active_m else '<span class="lang-flag">\U0001f1ea\U0001f1f8</span>ES'
