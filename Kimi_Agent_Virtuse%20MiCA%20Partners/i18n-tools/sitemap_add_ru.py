#!/usr/bin/env python3
"""
Add ru/<page> to sitemap.xml: inserts hreflang="ru" into every existing
<url> block for this page (the EN root entry, and the sk/, uk/, cs/
entries if they exist), then appends a new <url> block for ru/<page> with
the full set of alternates. Adapted from sitemap_add_cs.py for the 5th
language -- see i18n-tools/README.md "Adding the next language".

Usage: python3 sitemap_add_ru.py <page.html>
"""
import sys, re, os
...
# of language-folder prefix (root, sk/, uk/, cs/, ...).
...
if any('hreflang="ru"' in m.group(0) for m in matches):
    print("Already has ru entry for", page)
    sys.exit(0)
...
ru_hreflang_line = f'    <xhtml:link rel="alternate" hreflang="ru" href="https://staging.virtuse.com/ru/{page}"/>\n'

def insert_ru_line(block_text):
    marker = f'    <xhtml:link rel="alternate" hreflang="x-default" href="https://staging.virtuse.com/{page}"/>\n'
    if marker not in block_text:
        return block_text
    return block_text.replace(marker, ru_hreflang_line + marker, 1)

# Rebuild the file: walk matches in order, patch each block in place, and
# after the LAST matched block insert the brand-new ru/<page> block.
out = []
last_end = 0
for m in matches:
    out.append(s[last_end:m.start()])
    out.append(insert_ru_line(m.group(0)))
    last_end = m.end()
...
alt_lines.append(ru_hreflang_line)
...
ru_block = (
    f'  <url>\n'
    f'    <loc>https://staging.virtuse.com/ru/{page}</loc>\n'
...
s = s[:insert_at] + ru_block + s[insert_at:]

with open(SITEMAP, "w", encoding="utf-8") as f:
    f.write(s)
print("Updated sitemap for ru/" + page, f"(sk: {has_sk}, uk: {has_uk}, cs: {has_cs})")
