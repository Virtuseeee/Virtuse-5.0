#!/usr/bin/env python3
"""
One-time fixup, run exactly once after de/blog.html is created: flips every
de/*.html page's "Blog" links (nav item, and any hub/homepage blog teaser
link) from the temporary `../blog.html` (pointing at the English blog,
used while de/blog.html didn't exist yet) to the self-referencing
`blog.html` (resolves to the sibling de/blog.html).

Deliberately separate from relink_de.py, which only relinks page-to-page
...
pages = sorted(
    p for p in glob.glob(os.path.join(DE_DIR, "*.html"))
