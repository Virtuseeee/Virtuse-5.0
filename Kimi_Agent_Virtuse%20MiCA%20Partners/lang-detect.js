 ...
 * German, or French as its preferred language and they land on an
 * English page that has a matching translated counterpart, send them
 ...
 * ru -> ru/blog.html, de -> de/blog.html, fr -> fr/blog.html (all
 * UI-only shells over the English WP feed); cs -> no cs/blog.html
 * exists yet, so Czech browsers landing on blog.html are left alone.
 */
(function () {
  'use strict';
  ...
  try {
    ...
    if (!targetLang) return; // not a Slovak/Ukrainian/Czech/Russian/German/French-preferring browser

    var path = window.location.pathname;
    // Already on any translated section? No-op regardless of which one.
    if (path.indexOf('/sk/') !== -1 || path.indexOf('/uk/') !== -1 || path.indexOf('/cs/') !== -1 || path.indexOf('/ru/') !== -1 || path.indexOf('/de/') !== -1 || path.indexOf('/fr/') !== -1) return;
