...
— 19 cs pages** via `scratchpad/fix_low_cs.py` (clean first run).
- 36 of the 44 CS low items still applied; the secure/treasury/bots/about/
  faq anglicisms and the rainbow-chart title had been fixed in the medium
  round. **Kept on purpose:** "Bitcoin je váš kredit." (open EN headline
  decision), the tax table's "7+ EU" (EN keeps it).
- Notable: "oproti svému Bitcoinu" (Slovak sense) → "se zajištěním svým
  Bitcoinem" on index + lending; the lending hero now says Virtuse
  "zprostředkovává přístup ke službě" (same terms-page consistency fix as
  SK); rainbow-chart and root-cycles og/twitter descriptions no longer say
  "Rainbow Chart" (root-cycles described the wrong chart); root-cycles
  credit "@LeomathHeart (Bitcoin Wave)"; tax German link "Německy:
  Bitcoin-Steuern Deutschland"; index service cards + bots Cryptohopper
  blurb mirror the EN low round.
- Sitewide CS: hamburger aria-label "Otevřít menu"; cs/about got
...
**Round 14 of the proofreading pass (user: "oprav teraz low položky v SK")
— 24 files** via `scratchpad/fix_low_sk.py`.
- 50 of the 56 SK low items still applied (lending 5 %→6 %, root-cycles
  title, the "Tri jednoduché kroky" wording etc. were already done in
  earlier rounds). **Kept on purpose:** "Bitcoin je váš kredit." (same
  open headline decision as EN), the tax comparison table's "7+ EÚ"
  (EN keeps "7+ EU" there too).
- Judgement calls: lending "Virtuse sprostredkúva prístup k službe"
  (the old "poskytuje službu" contradicted the terms page); tax's
  German cross-link now reads "Po nemecky: Bitcoin-Steuern Deutschland"
  instead of a duplicate "Nemecko"; the index service cards mirror the
  EN low round (tax card "viac ako 30 krajín", mining card "od nákupu
  ASIC zariadení až po hosting v dátových centrách", "obchodných
  botov"); sk/bots Cryptohopper blurb mirrors EN's reworded one.
- Sitewide SK: hamburger `aria-label="Toggle menu"` → "Otvoriť menu"
  on all 22 SK pages incl. blog-sk; fear-greed/trading-volume titles
  use "|" like every other SK page; sk/about got EN's
  `.name-visual { overflow-x: clip }`. **SK has no newsletter fine
  print at all**, so the 15px fine-print bug does not apply to sk (the
  earlier "sk still 15px" note was wrong; fr, pl, hu remain).
- **Real link bug found and fixed (EN + sk):** on fear-greed and
  trading-volume the EN mobile language panel linked
  uk/cs/ru pages that don't exist (they 301 to the blog on
  production), and the EN desktop panel + hreflang lacked DE; the sk
  pages lacked DE entirely. Both now list EN, SK, FR, ES, PL, HU, DE.
  pl/ and hu/ versions of these two pages also lack a DE hreflang —
  left for the PL/HU session.
- Verified: tagcheck 0, no "Toggle menu" left, 17 SK pages at 375px
  with no horizontal overflow, sk/index concierge CTA fits (55–320px).

**Held production command:** superseded by the eighteenth-round entry above (173 files).
...
**Held production command:** superseded by the seventeenth-round entry above (155 files).
...
DE") — 23 de pages** via `scratchpad/fix_medium_de.py` (aborted once on a
count mismatch — "non-custodial Hub" also sits in meta descriptions that
the visible-text count missed; `git checkout -- de`, counts relaxed,
re-run clean).
- All medium items still open (register, "Keine PR", der Hub, H1 genders
  and the privacy "Virtuse Report" clause were closed in earlier rounds),
  plus: German closing quotes „…“ (16 pairs on aml/privacy/terms/rainbow),
  "–" in the footer Brief line, partner CTAs "<Partner> öffnen" for
  exchanges, tax software and bots ("holen" kept for Trezor/Ledger/Jade),
  "Steuerreporting" as the single term, Non-Custodial compounds.
- **Loan rate bug found while doing DE:** round 1 unified EN at 6 %, but
  the loan-type card ("ab 5 %") and the Firefish range ("5–15 %") on
  **every older translation's lending page** still said 5 % (pl/hu were
  built later from EN and were correct). Fixed in a separate commit
  (`5f78a80`) on sk, cs, fr, uk, es, ru; de in the DE commit.
- **Mobile overflow fixes:** de/buy-bitcoin overflowed 13px at 375px,
  because the single word "Gebührenranking" at 42px is wider than a phone.
  Added `hyphens: auto` + `overflow-wrap: break-word` for h1–h3 at
  ≤700px on all de pages (they declare lang="de"). de/index concierge CTA
  wraps on phones (overflowed 21px). All 22 de pages checked at 375px
  via iframes after the fix: no overflow except de/about (2px), which is
  **pre-existing on EN about too** (`.name-visual` decorative graphic is
  27px wider than its box) — not fixed, EN-source issue.
- Fine print 12px on de (the EN-source specificity bug; still open on
  EN, sk, fr, pl, hu).

**Held production deploy — everything from rounds 8–12 of the
proofreading pass (112 files: fr 22, uk 21, es 23, ru 21, de 23,
sk/cs lending).** All of them carry the PL/HU language-switcher entries,
and pl/ + hu/ still 301 on production. Ship pl/ + hu/ first (their own
rollout: `sftp mkdir public_html/pl` and `public_html/hu`, then their
pages plus the switcher wiring on EN/sk/cs/…), then run this. It
supersedes the per-language FR/UK/ES/RU commands in the entries below.
The file list lives at `scratchpad/touched_held_all.txt` for the md5 check.
```bash
cd /private/tmp/gh-pages-wt3 && \
